...
public class Converter {
    ...
    /**
     * @return the PID object in the current state. The parsing that have
     *         already happened define the content of the returned PID element
     */
    public PID getPid() {
	return pid;
    }
    ...
    /**
     * Preferred constructor. Constructor downloading the file from the online
     * repository, doing the conversion and outputting the results.
     * 
     * @param pidUrl
     * @param pathToOutputFile
     * @throws IOException
	...
	if (burger.tag("PathwayList")) {
		...
		if (burger.tag("Pathway")) {
		    ...
		    pathway.setSubnet(Boolean.parseBoolean(burger
			    .getTagAttribute("subnet")));
			...
			if (burger.tag("PathwayComponentList")) {
				...
				if (burger.tag("PathwayComponent")) {
				    ...
				    component
					    .setInteraction_idref(burger
						    .getTagAttribute("interaction_idref"));
				    pathway.getPathwayComponents().add(
					    component);
				}
			    }
	...
	if (burger.tag("InteractionList")) {
		...
		if (burger.tag("Interaction")) {
		    ...
		    String interactionType = burger
			    .getTagAttribute("interaction_type");
			...
			if (burger.tag("Abstraction")) {
			    ...
			    abstraction.setExternal_pathway_id(burger
				    .getTagAttribute("external_pathway_id"));
			    abstraction.setPathway_idref(Integer
				    .parseInt(burger
					    .getTagAttribute("pathway_idref")));
			    abstraction.setPathway_name(burger
				    .getTagAttribute("pathway_name"));
			...
			if (burger.tag("PositiveCondition")) {
			    ...
			    positiveCondition.setCondition_type(burger
				    .getTagAttribute("condition_type"));
			...
			if (burger.tag("NegativeCondition")) {
			    ...
			    negativeCondition.setCondition_type(burger
				    .getTagAttribute("condition_type"));
			...
			if (burger.tag("EvidenceList")) {
				...
				if (burger.tag("Evidence")) {
				    String evidence = burger
					    .getTagAttribute("value");
			...
			if (burger.tag("ReferenceList")) {
				...
				if (burger.tag("Reference")) {
				    int pmid = Integer.parseInt(burger
					    .getTagAttribute("pmid"));
			...
			if (burger.tag("InteractionComponentList")) {
				...
				if (burger.tag("InteractionComponent")) {
				    ...
				    String role = burger
					    .getTagAttribute("role_type");
				    int idref = Integer.parseInt(burger
					    .getTagAttribute("molecule_idref"));
				    ...
				    interactionComponent
					    .setMolecule_idref(idref);

				    while (burger.inTag("InteractionComponent")) {

					if (burger.tag("Label")) {
					    String labelType = burger
						    .getTagAttribute("label_type");
					    Label label = null;
					    if (labelType
						    .equals("activity-state")) {
					    ...
					    label.setValue(burger
						    .getTagAttribute("value"));
					    interactionComponent.getLabels()
						    .add(label);
					}
					...
					if (burger.tag("PTMExpression")) {
						...
						if (burger.tag("PTMTerm")) {
						    ...
						    ptmTerm.setAminoAcid(burger
							    .getTagAttribute("aa"));
						    ptmTerm.setModification(burger
							    .getTagAttribute("modification"));
						    ptmTerm.setPosition(Integer.parseInt(burger
							    .getTagAttribute("position")));
						    ptmTerm.setProtein(burger
							    .getTagAttribute("protein"));
						    ptmExpression.getPtmTerms()
							    .add(ptmTerm);
						}
					    }
					    interactionComponent
						    .setPtmExpression(ptmExpression);
					}
				    }
				    interaction.getComponents().add(
					    interactionComponent);
     ...
     * 
     */
    private void parseMolecules(Model model) throws NumberFormatException,
	    XMLStreamException {

	if (burger.tag("MoleculeList")) {
	    while (burger.inTag("MoleculeList")) {
		if (burger.tag("Molecule")) {
		    String moleculeType = burger
			    .getTagAttribute("molecule_type");
		    ...
		    molecule.setId(Integer.parseInt(burger
			    .getTagAttribute("id")));
		    while (burger.inTag("Molecule")) {
			if (burger.tag("Name")) {
			    Name name = null;
			    String nameType = burger
				    .getTagAttribute("name_type");
			    String longNameType = burger
				    .getTagAttribute("long_name_type");
			...
			if (burger.tag("ComplexComponentList")) {
				...
				if (burger.tag("ComplexComponent")) {
				    ...
				    complexComponent
					    .setMolecule_idref(Integer.parseInt(burger
						    .getTagAttribute("molecule_idref")));
				    while (burger.inTag("ComplexComponent")) {
					if (burger.tag("Label")) {
					    String labelType = burger
						    .getTagAttribute("label_type");
					    String labelValue = burger
						    .getTagAttribute("value");
					    Label label = null;
					    if (labelType
						    .equals("activity-state")) {
					    ...
					    complexComponent.getLabels().add(
						    label);
					}
					if (burger.tag("PTMExpression")) {
					    PTMExpression ptmExpression = new PTMExpression();
				    ...
				    molecule.getComplexComponents().add(
					    complexComponent);
				}
			    }
			}
			...
			if (burger.tag("FamilyMemberList")) {
				...
				if (burger.tag("Member")) {
				    ...
				    familyMember
					    .setMember_molecule_idref(Integer.parseInt(burger
						    .getTagAttribute("member_molecule_idref")));
			...
			if (burger.tag("Part")) {
			    ...
			    moleculepart
				    .setWhole_molecule_idref(Integer.parseInt(burger
					    .getTagAttribute("whole_molecule_idref")));
			    moleculepart
				    .setPart_molecule_idref(Integer.parseInt(burger
					    .getTagAttribute("part_molecule_idref")));
     ...
     * @throws NumberFormatException
     */
    private void parseOntology() throws NumberFormatException,
	    XMLStreamException {
	if (burger.tag("Ontology")) {
		...
		if (burger.tag("LabelType")) {
		    ...
		    labelType.setId(Integer.parseInt(burger
			    .getTagAttribute("id")));
			...
			if (burger.tag("LabelValue")) {
			    ...
			    labelValue.setId(Integer.parseInt(burger
				    .getTagAttribute("id")));
			    labelValue.setParent_idref(burger
				    .getTagAttribute("parent_idref"));
